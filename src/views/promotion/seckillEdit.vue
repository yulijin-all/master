      ...
      <el-form
        ...
        <el-form-item
          ...
          <el-radio
            ...
            :label="1"
            :disabled="disabled">允许</el-radio>
          <el-radio
            v-model="dataForm.repeatSeckill"
            :label="0"
            :disabled="disabled">不允许</el-radio>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card class="box-card">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="primary"
              size="mini"
              @click="handleProductShow(scope.row)">设置</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog
        ...
        <div
          ...
          <el-button
            type="primary"
            @click="handleProductEdit">确定</el-button>
        </div>
      </el-dialog>
    </el-card>
...
export default {
  ...
  methods: {
    init: function() {
      ...
      detailSeckill(goodsId).then(response => {
        const dataForm = response.data.data.goods
        this.dataForm = dataForm
        const seckillTime = this.dataForm.seckillTime
        const repeatSeckill = dataForm.repeatSeckill ? 1 : 0
        this.dataForm.repeatSeckill = repeatSeckill
        this.products = response.data.data.products
        this.disabled = this.isDuringDate(seckillTime[0], seckillTime[1])
    ...
    initBatchConfig() {
      getBatchConfig().then(response => {
        const batchList = response.data.data.batchList
        this.batchList = batchList
        const batchId = this.dataForm.batchId
        this.handleChange(batchId)
      })
    },
